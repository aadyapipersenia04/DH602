...
weights = torch.tensor([0.1, 0.3, 0.3, 0.1, 0.2],dtype=torch.float32).to(device)
device   
...
reward_model.load_state_dict(torch.load('results/saved_models/best_model.pth',map_location=device))
...
class TrainingConfig:
    ...
    output_dir = "/home/varad/aadya/finetuning"  # the model name locally and on the HF Hub
    temp_dir = "temp"  # where to save the model temporarily before using it with the reward model
    init_dir = "/home/varad/aadya/finetuning" # where the pretrained model is saved
