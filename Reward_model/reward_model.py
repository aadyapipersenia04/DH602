...
device = torch.device("cuda:1" if torch.cuda.is_available() else exit("GPU not available"))
print("Device:", device)
...
for epoch in range(num_epochs):
    ...
    for i in range(num_batches):
        ...
        rows_equal = np.sum(np.all(pred_np == labels, axis=1))
...
torch.save(model.state_dict(), 'results/saved_models/best_model.pth')
...
plt.savefig(f'results/losses_plot_resnext_{num_epochs}eps_1layerFc.png')
